...
### Access
- **URL**: `http://your-server:5000/` (or the port specified in `PORT` environment variable)
...
- **Read-only**: Dashboard cannot control bot or post tweets
- **No authentication**: Add authentication for production use
- **Cached data only**: No sensitive credentials displayed
- **Local access**: Accessible only from server network by default
- **No write operations**: All data is informational only
